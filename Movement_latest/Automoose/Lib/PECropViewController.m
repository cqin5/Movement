...
- (void)done:(id)sender
{
//    NSLog(@"%f %f",self.cropView.croppedImage.size.width,self.cropView.croppedImage.size.height);
...
-(void)setRatio{
    if([typeOfController isEqualToString:@"ProfilePhoto"])
    {
    ...
    else
    {
        ...
        CGFloat width = CGRectGetHeight(cropRect);
        cropRect.size = CGSizeMake(width, width * ratio);
        self.cropView.cropRect = self.view.bounds;
    }
}
