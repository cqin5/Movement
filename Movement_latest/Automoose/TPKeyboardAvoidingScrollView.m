...
- (void)keyboardWillShow:(NSNotification*)notification {
    ...
    self.contentInset = [self contentInsetForKeyboard];
    [self setContentOffset:CGPointMake(self.contentOffset.x, [self idealOffsetForView:firstResponder withSpace:[self keyboardRect].origin.y - self.bounds.origin.y]) animated:YES];
...
//    CGRect viewFrame = view.frame;
    ...
    CGFloat viewLocation = rect.origin.y+rect.size.height ;// Includes its height
    if(viewLocation < space)
    {
        if(space- viewLocation < 40)
            offset = 50;
        else
            offset = 0;
    }
        ...
        if(viewLocation > space)
        {
            CGFloat diff = viewLocation - space;
            offset = diff + 50;
        }
            /*
